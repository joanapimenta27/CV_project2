...
Here VGG-16 dominates on error metrics and EfficientNet-B0 achieves the highest accuracy.
Notably, Inception~v3 drops to last place , strong test performance paired with weak in-distribution results suggests training instability rather than overfitting, possibly exacerbated by the 4$\times$ oversampling strategy.
Training convergence curves are provided in Appendix~\ref{app:training_curves}.

\subsection{Grad-CAM Analysis}
We apply Grad-CAM~\cite{selvaraju2017gradcam} to the final convolutional layer of VGG-16 and Inception~v3 to interpret model focus during counting.
Representative heatmaps are provided in Appendix~\ref{app:gradcam}.
Both models attend primarily to ball regions, though VGG-16 produces broader activations while Inception~v3 yields more localised, ball-specific responses.
On failure cases, attention shifts to table edges or occluded areas.
